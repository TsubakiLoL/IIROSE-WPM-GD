...
"""
func _ready() -> void:
	%export.pressed.connect(_on_button_pressed)
func _on_button_pressed() -> void:
	%result.clear()
	var domin_name=%domin.text
	if domin_name=="":
		return
	if %is_with_main.button_pressed:
		%result.text=eval_main_js_code+js_code
	else:
		%result.text=js_code.replace("示例域名用于替换",domin_name)
